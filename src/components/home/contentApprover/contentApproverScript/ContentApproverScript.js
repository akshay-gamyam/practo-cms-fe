...
const ContentApproverScript = () => {
  ...
  const filteredScripts = contentApproverScripts.filter(script => {
    const matchesSearch = script.topic?.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         script.uploadedBy?.firstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         script.uploadedBy?.lastName?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const statusCounts = {
    ...
    pending: contentApproverScripts.filter(s => s.status?.toLowerCase() === 'pending' || s.status?.toLowerCase() === 'claimed' || s.status?.toLowerCase() === 'medical_review').length,
    approved: contentApproverScripts.filter(s => s.status?.toLowerCase() === 'approved').length,
    rejected: contentApproverScripts.filter(s => s.status?.toLowerCase() === 'rejected').length
  };
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500 font-medium">Version {script.version}</span>
                      </div>
                      <div>
                        {(script.status?.toLowerCase() === 'pending' || script.status?.toLowerCase() === 'claimed' || script.status?.toLowerCase() === 'medical_review') && !isClaimed && (
                          <button 
                        ...
                        {isClaimed && (script.status?.toLowerCase() === 'pending' || script.status?.toLowerCase() === 'claimed' || script.status?.toLowerCase() === 'medical_review') && (
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">{script.topic?.title || 'Untitled Script'}</h3>
                    ...
                    {script.status?.toLowerCase() === 'approved' && script.lockedById && (
                    ...
                    {script.status?.toLowerCase() === 'rejected' && script.lockedById && (
                    ...
                    {(script.status?.toLowerCase() === 'pending' || script.status?.toLowerCase() === 'claimed' || script.status?.toLowerCase() === 'medical_review') ? (
