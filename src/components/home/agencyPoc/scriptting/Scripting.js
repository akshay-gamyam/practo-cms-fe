...
const Scriptting = () => {
  ...
  const tabs = [
    ...
    },
    {
      id: "in-review",
      label: "In Review",
      count: scripts.filter((s) => s.status === "in-review").length,
    },
    {
      id: "rejected",
      label: "Rejected",
  ...
  const filteredScripts =
    activeTab === "all"
      ? scripts
      : scripts.filter((s) => s.status === activeTab);
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        {script.readyForVideo && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border bg-teal-50 text-teal-700 border-teal-200">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {script.videoUploaded && (
                        <button className="flex items-center gap-1 px-3 py-1.5 text-sm text-green-700 bg-green-50 border border-green-200 rounded hover:bg-green-100 transition-colors">
                      ...
                      {script.status === "locked" && script.readyForVideo && (
                        <button className="flex items-center gap-1 px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium text-white bg-purple-600 rounded hover:bg-purple-700 transition-colors">
                          <FiUpload size={14} />
                      ...
                      {script.status === "rejected" && (
                        <button className="flex items-center gap-1 px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium text-white bg-red-600 rounded hover:bg-red-700 transition-colors">
                          <FiEdit2 size={14} />
                      ...
                      {script.status === "draft" && (
                        <button className="flex items-center gap-1 px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700 transition-colors">
                          <FiEdit2 size={14} />
                      ...
                      {(script.status === "in-review" ||
                        ...
                        <button className="flex items-center gap-1 px-3 py-1.5 text-sm text-gray-700 hover:text-gray-900 transition-colors">
                          <FiEye size={14} />
                          <span className="hidden sm:inline">
                            View{script.status === "locked" ? " Script" : ""}
                          </span>
                        </button>
                      )}
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-purple-800 break-words">
                            ...
                            is now locked. You can proceed to upload the video.
                          </p>
                        </div>
                      </div>
                    </div>
